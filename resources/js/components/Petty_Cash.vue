<template>
  <div>
    ...
    <v-data-table v-if="requests" :headers="headers" :items="petty" :search="search" class="elevation-3">
      ...
      <template v-slot:item.approver_role.role_name="{ item }"> <v-chip class="ma-2" outlined :color="prpColor(item.approver_role.role_name)">{{item.approver_role.role_name === 'prp emp' ? 'PRP' : item.approver_role.role_name === 'finance mngr' ? 'Finance Manager' : item.approver_role.role_name === 'hr mngr' ? 'HR' : item.approver_role.role_name === 'general mngr' ? 'General Manager': '' }}</v-chip> </template>
      <template v-slot:item.actions="{ item }">
        <!-- <v-icon medium disabled class="mr-2" v-if="((user_type.includes('emp') && user_type.includes('prp emp') && user_type.includes('hr mngr')) && !item.approver_role.role_name === 'general mngr' && item.status.status_name === 'approved')">mdi-pencil</v-icon> -->
        <v-icon medium disabled class="mr-2" v-if="((user_type.includes('emp') && user_type.includes('prp emp')) && item.approver_role.role_name === 'general mngr')">mdi-pencil</v-icon>
        ...
        <!-- <v-icon medium disabled v-if="((user_type.includes('emp') && user_type.includes('prp emp') && user_type.includes('hr mngr')) && !item.approver_role.role_name === 'general mngr' && item.status.status_name === 'approved')">mdi-delete</v-icon>   -->
        <v-icon medium disabled v-if="((user_type.includes('emp') && user_type.includes('prp emp')) && item.approver_role.role_name === 'general mngr')">mdi-delete</v-icon>  
